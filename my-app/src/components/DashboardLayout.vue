...
.burger { display: none; position: fixed; top: 0; left: 0; z-index: 100; width: 100%; height: 52px; align-items: center; justify-content: flex-start; cursor: pointer; padding: 0 1rem; border: none; background: var(--bg-secondary); color: inherit; }
...
.sidebar {
  ...
  left: 0;
  height: 100vh;
  width: 240px; flex-shrink: 0; padding: 1rem 0; border-right: 1px solid var(--border-color);
  display: flex; flex-direction: column; transition: width 0.3s ease, transform 0.3s ease;
  background: var(--bg-secondary); z-index: 50;
  overflow-y: auto;
  overflow-x: hidden;
...
.nav-link {
  ...
  background: transparent; color: var(--text-secondary); font-size: 0.9375rem; cursor: pointer; text-align: left;
  transition: background 0.2s, color 0.2s; width: 100%; box-sizing: border-box; min-width: 0;
}
.nav-link:hover { background: var(--bg-hover); color: var(--text-primary); }
.nav-link.active { background: rgba(56, 189, 248, 0.15); color: var(--accent); }
...
.main { flex: 1; display: flex; flex-direction: column; min-width: 0; overflow: hidden; margin-left: 240px; transition: margin-left 0.3s ease; }
...
@media (max-width: 767px) {
  .burger { display: flex; }
  .sidebar { position: fixed; left: 0; top: 0; bottom: 0; width: 260px; transform: translateX(-100%); box-shadow: 4px 0 24px rgba(0,0,0,0.3); padding-top: 52px; }
